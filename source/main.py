...
class BridgerMacroApp:
    ...
    def setup_tesseract_tab(self):
        ...
        self.test_result_lbl = tk.Label(ocr_frame, text="Ready to test", font=("Arial", 7),
                                       relief="sunken", padx=10, pady=8)
        self.test_result_lbl.pack(fill="y", pady=5)
    ...
    def test_ocr(self):
        if not os.path.exists(tesseract_path):
            self.test_result_lbl.config(text=f"Tesseract not found: {tesseract_path}", fg="red")
            return
        
        left, top, right, bottom = self.bbox
        if right <= left or bottom <= top:
            self.test_result_lbl.config(text="Invalid bbox coordinates", fg="red")
            return
        
        try:
            detected_letter = self.process_image_for_ocr()
            self.test_result_lbl.config(text=f"", fg="black", font=("Arial", 10))
            
            if "tesseract not installed" in detected_letter.lower():
                self.test_result_lbl.config(text=f"{detected_letter}", fg="red")
            elif "no such file" in detected_letter.lower() or "cannot" in detected_letter.lower():
                self.test_result_lbl.config(text=f"Error: {detected_letter}", fg="red")
            elif detected_letter in ['T', 'G', 'F', 'R']:
                self.test_result_lbl.config(text=f" Detected: '{detected_letter}'", fg="green")
            else:
                self.test_result_lbl.config(text=f"No valid letter detected (Raw: '{detected_letter}')", fg="orange")
        except Exception as e:
            self.test_result_lbl.config(text=f" Exception: {str(e)[:50]}", fg="red")
